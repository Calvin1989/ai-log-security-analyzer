...
describe('i18n tool', () => {
  ...
  it('should have investigation view translations in both languages', () => {
    setLanguage('zh')
    expect(t('investigation.introDescription')).toBe('调查工作流概览')
    expect(t('investigation.introBody')).toBe('在同一个调查工作区中复核事件时间线、关联事件、发现项和规则覆盖情况。')
    expect(t('investigation.timelineIncidentsTitle')).toBe('时间线与事件')
    expect(t('investigation.timelineIncidentsDescription')).toBe('还原事件顺序并复核关联事件上下文。')
    expect(t('investigation.findingsRulesTitle')).toBe('发现项与规则')
    expect(t('investigation.findingsRulesDescription')).toBe('结合规则覆盖和调优上下文复核检测结果。')
    expect(/[一-龥]/.test(t('investigation.introBody'))).toBe(true)
    expect(/[一-龥]/.test(t('investigation.timelineIncidentsTitle'))).toBe(true)
    expect(/[一-龥]/.test(t('investigation.findingsRulesTitle'))).toBe(true)

    setLanguage('en')
    expect(t('investigation.introDescription')).toBe('Investigation workflow overview')
    expect(t('investigation.introBody')).toBe('Review the incident timeline, correlated incidents, findings, and rule coverage in one investigation workspace.')
    expect(t('investigation.timelineIncidentsTitle')).toBe('Timeline & Incidents')
    expect(t('investigation.timelineIncidentsDescription')).toBe('Reconstruct event order and review grouped incident context.')
    expect(t('investigation.findingsRulesTitle')).toBe('Findings & Rules')
    expect(t('investigation.findingsRulesDescription')).toBe('Review detections alongside rule coverage and tuning context.')
  })
})
